...
def find_largest(img, threshold='opening'):
    ...
    -   'opening':   applies global Otsu thresholding after a minor
                     morphological opening (default)
    ...
    -   'rag':       applies RAG thresholding, followed by global Otsu.
                     Slowest method.

    Coming soon:
    -   'textural':  eliminates backgrounds of (any) constant colour based
                     on GLCM or binary patterns
...
def auto_canny(img, sigma=0.33):
    ...
    Original function from pyimagesearch: Zero-parameter, automatic Canny
    edge with with Python and OpenCV
...
def find_data(img):
    ...
    """
    # Make sure we have the kind of array we want.
    img = np.asarray(img)[..., :3]

    # Carry on.
    mask = find_largest(img)
    warped = rectify_seismic(img, mask)
    clean = remove_annotations(warped)

    # Cast back to PIL Image and go back.
    return Image.fromarray(np.uint8(clean*255))
